...
const SeeItems = ({ showToast }) => {
  ...
  const loadAllData = () => {
    try {
      ...
      // Set dummy statistics data
      setStats({
        totalCollections: 247,
        processedBatches: 189,
        testedBatches: 156,
        pendingProcessing: 8,
        pendingTesting: 3
      });

      // Set empty data arrays to remove items display
      setAllData([]);
      setFilteredData([]);
      setIsLoading(false);
                        ...
                        <div className="mb-6">
                          ...
                          <div className="flex flex-wrap gap-2">
                            {item.processingSteps.map((step, stepIndex) => (
                              <div key={stepIndex} className="flex items-center gap-2 px-3 py-1 bg-blue-500/20 border border-blue-500/30 rounded-full">
                                <Factory className="w-3 h-3 text-blue-400" />
                                <span className="text-blue-300 text-sm font-medium">{step.stepType}</span>
                              </div>
                            ))}
                          </div>
                        </div>
                        ...
                        <div>
                          ...
                          <div className="flex flex-wrap gap-2">
                            {item.labTests.map((test, testIndex) => (
                              <div key={testIndex} className="flex items-center gap-2 px-3 py-1 bg-green-500/20 border border-green-500/30 rounded-full">
                                <Beaker className="w-3 h-3 text-green-400" />
                                <span className="text-green-300 text-sm font-medium">{test.testType}</span>
                                {test.status && (
                                  <>
                                    <span className="w-1 h-1 bg-gray-500 rounded-full"></span>
